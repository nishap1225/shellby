...
def main():
    ...
    print("Enter the first four angles of the Sawyer's start state, space separated: ")
    s_angles = input().split(' ')
    s_angles = [float(a) for a in s_angles]
    s_angles.extend([0.0, 0.0, 0.0])

    goal.request.start_state.joint_state.name = joint_names
    goal.request.start_state.joint_state.position = s_angles
    ...
    print("Enter the first four angles of the Sawyer's goal state, space separated: ")
    e_angles = input().split(' ')
    e_angles = [float(a) for a in e_angles]
    e_angles.extend([0.0, 0.0, 0.0])

    arm_joint_names = joint_names
    target_joint_angles = e_angles
    tolerance = 0.0001
    consts = []

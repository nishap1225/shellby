...
def main():
    ...
    pose = PoseStamped(pose=Pose(position=Point(x=0.5, y=0.0, z=0.0), orientation=Quaternion(x=0.0, y=0.0, z=0.0, w=1.0)))
    ...
    while not rospy.is_shutdown():
        ...
        while not rospy.is_shutdown():
            try:
                ...
                plan = planner.plan_to_pose(goal_1, []) # pass in orien_const for the constraint 
        ...
        while not rospy.is_shutdown():
            try:
                ...
                plan = planner.plan_to_pose(goal_2, []) #orient_const 

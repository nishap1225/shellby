...
from moveit_msgs.msg import OrientationConstraint, PositionConstraint, Constraints, CollisionObject
...
class PathPlanner(object):
    ...
    #TODO: add position constraint to path
    ...
    def add_cylinder_obstacle(self, size, name, pose):
        """
        Adds a cylinder obstacle to the planning scene

        Inputs:
        size: 2x' ndarray; (height, radius) size of the box (in the box's body frame)
        name: unique name of the obstacle (used for adding and removing)
        pose: geometry_msgs/PoseStamped object for the CoM of the box in relation to some frame
        """    
